...
function wireHamSection(){
  ...
  function run(){
    ...
    const { frame, total, rPositions } = buildHammingFrame(bits);
    ...
    const flipPos = parseInt(flipSelect.value || "0", 10);
    ...
    if(flipPos > 0){
      ...
      frameEl.innerHTML += `<div style="margin:14px 0 6px; font-size:11.5px; color:var(--ink-dim);">Trama recibida (con error simulado en la posición ${flipPos}):</div>`;
    ...
    if(syndrome > 0){
      ...
      frameEl.innerHTML += `<div style="margin:14px 0 6px; font-size:11.5px; color:var(--phosphor-dim);">Trama corregida (bit ${syndrome} invertido):</div>`;
    ...
    readoutEl.innerHTML = `
      <div><div class="k">Trama enviada (${total} bits)</div><div class="v good">${frameToString(frame, total)}</div></div>
      <div><div class="k">Bits de redundancia</div><div class="v">${rPositions.join(", ")}</div></div>
