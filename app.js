...
function wireHamSection(){
  ...
  let lastTotalForSelect = null; // recuerda el total con el que se pobló el <select> por última vez
  ...
  function dataOnlyRowHtml(frame, total, highlightPositions, labelFn){
    // Muestra ÚNICAMENTE las posiciones de datos (tipo "d"), sin los bits r,
    // en el mismo orden de lectura (posición alta a baja). El encabezado
    // numera el bit de dato (1°, 2°, 3°...) en vez de la posición física.
    const dataPositions = [];
    for(let p=total; p>=1; p--) if(frame[p].type === "d") dataPositions.push(p);

    let html = `<table class="bittable"><tr>`;
    dataPositions.forEach((p, i) => {
      html += `<th>${labelFn ? labelFn(i+1, p) : (i+1)}</th>`;
    });
    html += `</tr><tr>`;
    dataPositions.forEach(p => {
      let cls = "d-bit";
      if(highlightPositions && highlightPositions.has(p)) cls += " changed";
      html += `<td class="${cls}">${frame[p].value}</td>`;
    });
    html += `</tr></table>`;
    return html;
  }
  ...
  function run(){
    ...
    const { frame } = buildHammingFrame(bits);

    if(mode === "manual"){
      // OJO: repoblar el <select> en cada run() borraba la selección del
      // usuario justo antes de leerla (porque el evento "change" dispara
      // run(), y run() volvía a llamar populateFlipOptions). Por eso el
      // modo manual "no servía": el valor se resetea a "Ninguno" antes
      // de poder usarse. Ahora solo se repuebla si cambió el total de bits
      // (por ejemplo, al editar la cadena de datos), preservando la opción
      // elegida en cualquier otro caso.
      if(lastTotalForSelect !== total){
        populateFlipOptions(total);
        lastTotalForSelect = total;
      }
    } else {
    ...
    frameEl.innerHTML = `
      <div style="margin-bottom:16px; padding:12px 14px; border:1px solid var(--line); background:var(--bg);">
        ...
        <div style="font-size:12px; color:var(--ink-dim); margin-top:8px;">
          m = ${m} bits de datos &nbsp;|&nbsp; r = ${r} bits de redundancia (uso interno, no se muestran) &nbsp;|&nbsp; total transmitido = <strong style="color:var(--ink);">${total} bits</strong>
        </div>
      </div>
    `;
    ...
    // ---- Solo se muestra UNA tabla: los bits de datos, YA corregidos ----
    // Si el bit dañado caía en un dato, se resalta esa posición porque
    // ese es el bit que se corrigió. Si caía en un bit r (invisible aquí),
    // no hay nada que resaltar porque los datos nunca se tocaron.
    const correctedFrameTyped = {};
    for(let p=1;p<=total;p++) correctedFrameTyped[p] = { value: correctedFrame[p], type: frame[p].type };

    const errorEnDato = flipPos > 0 && frame[flipPos].type === "d";
    const highlightSet = errorEnDato ? new Set([flipPos]) : new Set();

    frameEl.innerHTML += `<div style="margin-bottom:8px; font-size:11.5px; color:var(--phosphor-soft);">Bits de datos — ya corregidos${errorEnDato ? " (el bit en rojo fue el dañado y reparado)" : ""}:</div>`;
    frameEl.innerHTML += dataOnlyRowHtml(correctedFrameTyped, total, highlightSet);
    ...
    // ---- Trama final SOLO con los datos (sin bits de redundancia) ----
    const soloDatosCorregidos = dataOnlyString(correctedFrameTyped, total);

    const receivedFrameTyped = {};
    for(let p=1;p<=total;p++) receivedFrameTyped[p] = { value: receivedFrame[p], type: frame[p].type };
    const soloDatosRecibidos = dataOnlyString(receivedFrameTyped, total);

    readoutEl.innerHTML = `
      <div><div class="k">Bits de datos (m)</div><div class="v">${m} bits</div></div>
      <div><div class="k">Trama recibida — datos (con error, si lo hay)</div><div class="v ${flipPos>0 && errorEnDato ? 'bad':''}">${soloDatosRecibidos}</div></div>
      <div><div class="k">Trama corregida — solo datos ✓</div><div class="v good">${soloDatosCorregidos}</div></div>
  ...
  lastTotalForSelect = initial.total;
  syncModeVisibility();
  run();
}
